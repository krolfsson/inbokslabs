    ...
    >
      ...
      {aiNoKey ? (
        <p className="mt-4 rounded-xl bg-zinc-50 px-3 py-2.5 text-[12px] leading-snug text-zinc-600">
          ...
          (lokal dev) eller i Vercel Environment Variables för produktion. Tills dess
          visas regelmodellen nedan.
        </p>
      ) : null}
      ...
      {showAiBlock && !aiError && !aiNoKey ? (
        <div className="mt-4 border-t border-zinc-100 pt-4">
          ...
          {aiBusy && aiRaw.length === 0 ? (
            <p className="mt-2 text-[12px] text-zinc-500">Ansluter till modellen…</p>
          ) : (
            <div className="mt-2 min-h-[3rem] whitespace-pre-wrap text-[12px] leading-relaxed text-zinc-700">
              {parsed.analysis.trim() ? (
                ...
                aiRaw.length === 0 ? (
                  ...
                  <span className="text-zinc-500">Analyserar första raden (POÄNG)…</span>
                )
              ) : (
